...
|---|---|---|---|
| `kerbe.planning_root` | path | yes | Slice folders live at `{planning_root}/{slice}` relative to the project root; the slice registry is `{planning_root}/INDEX.md`. The path may leave the code repo (`../mgr-pm/planning/slices`): a relative path resolves the same from the main checkout and from every sibling worktree. The **planning repo** is the git repo containing the resolved root — `git -C {planning_root} rev-parse --show-toplevel` — and every planning commit runs there: `git -C {planning_repo} commit -m "…" -- <path relative to {planning_repo}>`. When planning and code share a repo the two are the same and nothing changes. |
...
| `kerbe.editor_cmd_method` | command template | optional | Method-anchored variant used for review rows that name a `method()` — `{method}` and `{file}` (absolute) are substituted; the template resolves the line at run time so it survives line drift (e.g. `phpstorm --line $(grep -n 'function {method}(' {file} \| cut -d: -f1) {file}` for PHP — the grep is stack-specific, which is why it lives in project config). Unset ⇒ every row uses `editor_cmd`. |
...
| `kerbe.stack.exec` | command template | optional | Wrapper for every command an adapter's `commands.md` defines — `{cmd}` is substituted, and `{slice}` interpolates like it does in `code_roots` (e.g. a per-slice container exec). `{lane}` interpolates the lane index when the project runs more than one verification lane (see `workspace.lanes`). Unset ⇒ commands run directly in the workspace. |
...
| `kerbe.workspace.lanes` | int | optional (default `1`) | How many isolated **verification lanes** this project can run for one slice. A lane is a worktree plus whatever it takes to run the tests against it — container, port, test database. Lane 0 is the slice workspace itself. `1` ⇒ every lane-bound task runs in the workspace, one at a time. With `lanes > 1`, `stack.exec` **must** contain `{lane}`, or every lane would route its commands to the same environment and a worker could edit files while verifying nothing — that combination is a hard stop naming this key. |
| `kerbe.workspace.lane_setup_cmds` | list of strings | with `lanes > 1` | Commands run **once** when a lane is created, to stand up its own environment (container, database, seed data). `{lane}` interpolates the lane index; `{slice}` interpolates as elsewhere. kerbe does no port or container-name arithmetic — the template owns it, which is what keeps the lane model portable to stacks that need no containers at all. |
| `kerbe.workspace.lane_teardown_cmds` | list of strings | optional | Commands run when the slice's run ends, to remove a lane's environment. Lane 0 is never torn down — it is the workspace. |
| `kerbe.workspace.worktree_setup_cmds` | list of strings | optional | Commands run once in **every** fresh worktree, lane-bound or not — the language's dependency install (`composer install`, `dart pub get`). Distinct from `lane_setup_cmds` because a lane-free worker needs this and nothing else. **A fresh git worktree has source and no `vendor/`**, so without this key even a pure-unit worker cannot run its test command; unset ⇒ `kerbe:implement` runs every task in lane 0 and says why. |
